...
export const obtenerTodos = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ estado: true, mensaje: "Turnos encontrados.", turnos });
  } catch (error) {
...
export const obtenerUno = async (req, res) => {
  try {
    ...
    if (!filas.length)
      return res
        .status(404)
        .json({ estado: false, mensaje: "Turno no encontrado o inactivo" });
    res
      .status(200)
      .json({ estado: true, mensaje: "Turno encontrado.", turno: filas[0] });
  } catch (error) {
  ...
  try {
    ...
    if (error.message === "MEDICO_NOT_FOUND") {
      return res
        ...
        .json({
          estado: false,
          mensaje: "El médico seleccionado no existe o está inactivo.",
        });
    }
    if (error.message === "OS_NOT_FOUND") {
      return res
        ...
        .json({
          estado: false,
          mensaje: "La obra social seleccionada no existe o está inactiva.",
        });
    }
    if (error.code === "ER_NO_REFERENCED_ROW_2") {
      return res
        ...
        .json({
          estado: false,
          mensaje:
            "Verifique los IDs de médico, paciente u obra social provistos.",
        });
    }
    ...
    res
      ...
      .json({
        estado: false,
        mensaje: "Error interno de servidor al procesar la transacción.",
      });
  }
};
...
export const actualizar = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ estado: true, mensaje: "Turno modificado correctamente." });
  } catch (error) {
...
export const eliminar = async (req, res) => {
  try {
    ...
    if (!resultado.affectedRows)
      return res
        .status(404)
        .json({ estado: false, mensaje: "Turno no encontrado o ya cancelado" });
    res
      .status(200)
      .json({ estado: true, mensaje: "Turno cancelado (Baja lógica)." });
  } catch (error) {
